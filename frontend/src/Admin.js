...
const Admin = () => {
			...
			try {
				...
				const modifiedData = dashboardData.map((course) => {
					const totalScore = categories.reduce(
						(acc, category) =>
							acc +
							course.categories.find((cat) => cat.category === category)
								.totalScore,
						0
					);
					const avg =
						course.totalStudents !== 0
							? Math.round(totalScore / course.totalStudents)
							: 0;
	...
	const columns = [
		...
		...categories.map((category) => ({
			...
			render: (text, record) =>
				record.categories.find((cat) => cat.category === category).totalScore, // Render the totalScore for each category
		})),
		{
